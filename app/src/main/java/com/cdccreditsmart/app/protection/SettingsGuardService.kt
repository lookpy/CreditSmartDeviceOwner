...
class SettingsGuardService(private val context: Context) {
    ...
    private suspend fun checkAndInterceptBlockedApp(packageName: String): Boolean {
        ...
        // Usa métodos leves (SharedPreferences) para não causar erro no provisionamento
        // ═══════════════════════════════════════════════════════════════════════════════
        try {
            val currentLevel = appBlockingManager.getCurrentBlockingLevel()
            val hasManualBlock = appBlockingManager.hasManualBlock()
            if (currentLevel == 0 && !hasManualBlock) {
                return false
            }
        } catch (e: Exception) {
