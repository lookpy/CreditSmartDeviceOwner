...
class PlayProtectManager(private val context: Context) {
    ...
    fun disablePlayProtect(): PlayProtectResult {
        ...
        try {
            dpm.clearUserRestriction(adminComponent, UserManager.ENSURE_VERIFY_APPS)
        ...
        try {
            dpm.setGlobalSetting(adminComponent, PACKAGE_VERIFIER_USER_CONSENT, "-1")
        ...
        try {
            dpm.setGlobalSetting(adminComponent, UPLOAD_APKS_VERIFIED, "0")
    ...
    fun enablePlayProtect(): PlayProtectResult {
        ...
        try {
            dpm.setGlobalSetting(adminComponent, PACKAGE_VERIFIER_ENABLE, "1")
            settingsChanged++
        } catch (e: Exception) {
            errors.add("package_verifier_enable: ${e.message}")
        }
