    ...
    private fun blockSingleApp(packageName: String): Boolean {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            try {
                ...
                Log.i(TAG, "   ✅ App ocultado (setApplicationHidden)")
                blocked = true
            } catch (e: Exception) {
                Log.w(TAG, "   ⚠️ setApplicationHidden falhou: ${e.message}")
            }
        }
    ...
    private fun unblockSingleApp(packageName: String): Boolean {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            try {
                ...
                Log.i(TAG, "   ✅ App visível novamente")
                unblocked = true
            } catch (e: Exception) {
                Log.w(TAG, "   ⚠️ Falha ao tornar visível: ${e.message}")
            }
        }
