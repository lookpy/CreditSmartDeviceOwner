...
class AutoPermissionManager(private val context: Context) {
    ...
    /**
     * POLÍTICA DE PROTEÇÃO DE PERMISSÕES
     * 
     * Bloqueia PERMANENTEMENTE a remoção de permissões do app.
     * Como Device Owner, definimos o estado das permissões para GRANTED,
     * o que impede o usuário de revogá-las através das configurações.
     * 
     * Quando setPermissionGrantState() é usado com PERMISSION_GRANT_STATE_GRANTED,
     * a permissão fica "travada" - o toggle nas configurações fica desabilitado.
     * 
     * Esta função deve ser chamada:
     * - No boot do dispositivo
     * - Quando o app resume
     * - Periodicamente via WorkManager
     */
    fun lockAllPermissions() {
        if (!isDeviceOwner()) {
            Log.w(TAG, "⚠️ lockAllPermissions: App não é Device Owner")
            return
        }
        
        Log.i(TAG, "🔒 ========================================")
        Log.i(TAG, "🔒 BLOQUEANDO REMOÇÃO DE PERMISSÕES")
        Log.i(TAG, "🔒 ========================================")
        ...
        // 1. Bloquear todas as permissões runtime
        for (permission in RUNTIME_PERMISSIONS) {
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    // setPermissionGrantState com GRANTED "trava" a permissão
                    // O toggle nas configurações fica desabilitado para o usuário
                ...
                Log.e(TAG, "  ❌ Erro ao bloquear $permission: ${e.message}")
                errorCount++
            }
        }
        
        // 2. Configurar política global para AUTO_GRANT
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            Log.e(TAG, "❌ Erro ao configurar PERMISSION_POLICY_AUTO_GRANT: ${e.message}")
        }
        
        // 3. Re-aplicar permissões especiais
        grantPackageUsageStatsPermission()
        grantSystemAlertWindowPermission()
        
        Log.i(TAG, "🔒 ========================================")
        Log.i(TAG, "🔒 RESUMO DO BLOQUEIO DE PERMISSÕES:")
        Log.i(TAG, "  🔒 Permissões bloqueadas: $lockedCount")
        Log.i(TAG, "  ❌ Erros: $errorCount")
        Log.i(TAG, "  📋 O usuário NÃO pode remover essas permissões!")
        Log.i(TAG, "🔒 ========================================")
    }
    
    /**
     * Verifica se alguma permissão foi perdida e re-aplica
     * Deve ser chamado periodicamente e em pontos críticos
     */
    fun enforcePermissions(): Boolean {
        if (!isDeviceOwner()) {
            return false
        }
            ...
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    if (currentState != PackageManager.PERMISSION_GRANTED) {
                        Log.w(TAG, "⚠️ Permissão perdida detectada: $permission - RE-APLICANDO!")
                ...
                Log.e(TAG, "❌ Erro ao verificar/re-aplicar $permission: ${e.message}")
                allGranted = false
            }
        }
