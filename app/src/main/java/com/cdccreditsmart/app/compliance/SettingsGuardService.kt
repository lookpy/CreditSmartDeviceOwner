...
class SettingsGuardService(private val context: Context) {
     ...
     * 
     * Método: DISABLED for Play Protect compliance (was: setApplicationHidden toggle)
     * 
    ...
    private suspend fun forceStopBlockedApp(packageName: String): Boolean {
        ...
        // ═══════════════════════════════════════════════════════════════════════════════
        // MÉTODO 1: setApplicationHidden toggle - DISABLED for Play Protect compliance
        // ═══════════════════════════════════════════════════════════════════════════════
        Log.i(TAG, "⚠️ Feature disabled: setApplicationHidden toggle - Play Protect compliance")
        Log.i(TAG, "   ℹ️ Would have force-stopped: $packageName")
            ...
            if (dpm?.isDeviceOwnerApp(context.packageName) == true) {
                ...
                try {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        PolicyHelper.setPackagesSuspended(dpm, adminComponent, settingsPackages, true)
                        
                        mainHandler.postDelayed({
                            try {
                                PolicyHelper.setPackagesSuspended(dpm, adminComponent, settingsPackages, false)
                            } catch (e: Exception) {
