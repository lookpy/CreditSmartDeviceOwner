...
### 2025-12-09: Correção de Execução em Background (Release + Device Owner)

**Problema Identificado:**
- Em debug, tudo funcionava (guard, heartbeat, comandos MDM)
- Em release com Device Owner, o app era morto pelo sistema em background

**Causa Raiz:**
1. Certificate pinning com pins PLACEHOLDER bloqueava conexões em release
2. App não estava isento de otimização de bateria
3. Lock Task packages não configurados

**Correções Implementadas:**

**1. CertificatePinningManager.kt:**
- Desabilitado certificate pinning para domínios CDC em TODAS as builds
- Corrigido verificação de domínio de `.com.br` para `.com`

**2. WorkPolicyManager.ensureBackgroundExecutionPrivileges():**
- Nova função que configura Lock Task packages
- Configura Lock Task features (notificações, home, global actions)
- Verifica isenção de bateria
- Marca app como crítico (uninstall blocked)

**3. CDCApplication.ensureBackgroundExecutionPrivilegesIfDeviceOwner():**
- Chamada ANTES de iniciar o ForegroundService
- Garante que privilégios estão configurados

**4. AutoPermissionManager.requestBatteryOptimizationExemption():**
- Tenta adicionar à whitelist via shell commands
- Fallback para diálogo do usuário

**Garantias:**
- ForegroundService não é morto em background
- Conexões de rede funcionam em release
- SettingsGuard e HeartbeatManager continuam ativos