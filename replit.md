...
- **Samsung Knox Lockscreen Overlay**: Automatic lockscreen messages on Samsung Knox devices showing personalized warnings based on overdue days (7, 15, 30, 45, 60 days). Uses `KnoxLockscreenManager` to apply messages via `com.samsung.android.knox.lockscreen.LockscreenOverlay` API. Features include custom messages per level, transparency control (alpha 0.8f), emergency phone configuration (190), and automatic reset on unblock. Graceful fallback on non-Knox devices (lockscreen updates are skipped but app continues working normally).
