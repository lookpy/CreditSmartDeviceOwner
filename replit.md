...
- **Device Owner Management:** Manages Device Owner status, including brand-specific provisioning and auto-configuration to apply policies immediately. Automatically sets system launcher as default using `DevicePolicyManager.addPersistentPreferredActivity()` to eliminate manual launcher selection prompt during provisioning (21/11/2025).
...
- **Post-Factory-Reset Enrollment (Hybrid):** ✅ **IMPLEMENTADO** - Sistema completo de reinstalação automática do APK após factory reset via Samsung Knox Mobile Enrollment (KME) para Samsung e Android Zero-Touch Enrollment para outros fabricantes (Infinix, Motorola, Xiaomi, etc). `EnrollmentManager` orquestra `KnoxEnrollmentHelper` e `ZeroTouchHelper` para detectar tipo de enrollment, coletar informações do device e reportar status ao backend via `POST /api/enrollment/report`. Integrado ao `AutoProvisioningReceiver` para detecção automática durante setup. **REQUER CONFIGURAÇÃO EXTERNA:** (1) Cadastro de IMEIs no portal Google Zero-Touch ou Samsung Knox, (2) Backend hosting APK em `GET /app/download`, (3) Endpoint de enrollment report. Ver `docs/setup/ZERO_TOUCH_SETUP_GUIDE.md` para guia prático completo e `docs/features/ENROLLMENT_GUIDE.md` para procedimentos operacionais detalhados.
- **QR Code Provisioning:** QR Code contains complete JSON with provisioning configuration. Component name: `com.cdccreditsmart.app/com.cdccreditsmart.device.CDCDeviceAdminReceiver`. Backend endpoint `https://api.cdccreditsmart.com/app/?code=provision` serves APK binary directly (not JSON). Requires signed release APK with signature checksum (SHA-256 of certificate, not APK file) in base64url format via keytool. See `docs/qr-code/QR_CODE_PROVISIONING_GUIDE.md` for complete setup guide and `docs/qr-code/QR_CODE_BACKEND_INTEGRATION.md` for backend implementation.
- **Anti-Removal Protections:** `AppProtectionManager` provides multi-layered defenses against uninstallation, force stops, data clearing, factory resets, and Device Admin removal. Includes `getBlockedActions()` and `logBlockedActions()` for honest documentation of applied policies, separating **✅ GUARANTEED** (factory reset via Settings, uninstall, force stop, accounts, users), **⚠️ ATTEMPTED** (VPN, networks, Knox), and **❌ NON-BLOCKABLE** (recovery mode/fastboot) protections. **DEBUG MODE:** ADB debugging and USB file transfer remain active in debug builds (`BuildConfig.DEBUG = true`) to enable development/testing, automatically blocked in release/production builds (21/11/2025).
...
- **CDC Credit Smart Backend API:** For APK authentication, auto-discovery, device status, installments, PIX payment processing, heartbeat, MDM commands, unblock operations, remote uninstall telemetry, time synchronization, FCM token registration, and enrollment reporting. MDM endpoints prioritize IMEI > Serial Number > Device ID. **CRÍTICO:** Must host APK download endpoint (`GET /app/download`) serving signed APK binary for Knox KME and Zero-Touch provisioning auto-reinstallation after factory reset. Must also implement enrollment report endpoint (`POST /api/enrollment/report`).
