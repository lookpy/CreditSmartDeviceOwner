...
- **Progressive Blocking System:** Receives MDM commands to progressively block applications based on overdue levels, using `setPackagesSuspended()` to keep app icons visible while blocking access. **Intelligent filtering blocks user-installed apps AND pre-installed apps (YouTube, Chrome, Google Photos, etc.) while protecting only essential system apps (SystemUI, Providers, Telefone, SMS).** Icons remain visible in the launcher, providing visual reminder of blocked apps. **Universal Overlay System:** `BlockedAppInterceptor` monitors ALL foreground apps and displays a custom CDC overlay in ANY app when there are overdue installments (blockingLevel > 0), showing payment information, contact details, and PIX payment options. The overlay is **fully dismissible** with multiple closing options (X button in TopBar, large close button, Android back button) to ensure emergency calls and essential device functions remain accessible. **Dual Cooldown System:** Global cooldown (60 seconds) prevents overlay spam when switching between apps, while per-app cooldown (5 seconds) prevents repeated display in the same app. This ensures the overlay appears maximum once per minute regardless of app switching, maintaining effectiveness without frustration. **Modern UI Design:** Clean card-based layout with hero header (lock icon), highlighted total overdue amount, expandable installment details with smooth animations, hierarchical CTAs (PIX payment primary, contact secondary), and conditional manual block reason card for administrative blocks. **Periodic Overlay System (WorkManager):** Sistema de overlay automático periódico que NÃO requer permissão PACKAGE_USAGE_STATS. Usa WorkManager para agendamento com intervalo progressivo baseado em dias de atraso: 10min (1-4 dias), 5min (5-14 dias), 3min (15+ dias). Overlay imediato aparece INSTANTANEAMENTE quando bloqueio é aplicado via `showImmediateOverlay()`, depois continua nos intervalos progressivos. Sistema de notificação prévia mostra alerta 1 minuto antes de cada overlay subsequente (exceto primeiro). Overlay é totalmente fechável garantindo acesso a emergências. **Compliance Verification System (Otimizado):** `HeartbeatWorker` reporta estado de bloqueio ao backend a cada 1 hora (otimizado de 15min = redução de 75% nas requisições). Sistema inteligente com cache de estado usa hash para detectar mudanças e SKIP envio quando estado permanece inalterado, economizando ~50-70% adicionais. Backend valida conformidade e retorna complianceStatus. Se NON_COMPLIANT, APK corrige automaticamente via `forceComplianceCorrection()` com proteção contra loops infinitos (máx 3 correções consecutivas). BlockingCheckWorker removido (redundante - economia de ~192 req/dia/dispositivo).
