...
- **Financial Features:** Progressive blocking of non-essential applications based on payment status using `setApplicationHidden()`. Includes a universal dismissible overlay for overdue reminders and an integrated PIX payment system.
...
- **App Blocking Logic:** Includes a `CRITICAL_NEVER_BLOCK_PACKAGES` list and `safeSetApplicationHidden()` wrapper. `SettingsGuardService` intercepts blocked apps using `UsageStats` and `BlockedAppExplanationActivity` with throttling. The system also detects and closes blocked apps in split screen mode.
...
- **Permission Handling:** The PermissionGateScreen explicitly shows missing permissions for manual granting, no longer bypassing for Device Owner mode.
