...
- **Progressive Blocking System:** Receives MDM commands to progressively block applications based on overdue levels. **Uses BlockedAppInterceptor EXCLUSIVELY** - apps remain functional (not suspended) with icons visible in the launcher. When client attempts to open a blocked app, the app launches normally but BlockedAppInterceptor detects it within 1-2 seconds, closes the app, and displays BlockedAppExplanationActivity with customized CDC messaging including PIX payment information, overdue days, and payment options. **Intelligent filtering blocks user-installed apps AND pre-installed apps (YouTube, Chrome, Google Photos, etc.) while protecting only essential system apps (SystemUI, Providers, Telefone, SMS).** Polling is optimized: 2s normal, 1s aggressive after detections, 60s when screen is off for battery optimization. System pauses monitoring when no apps are blocked. Optional BlockedAppAccessibilityService available for instant interception when manually enabled by user.
