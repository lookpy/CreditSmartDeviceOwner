...
- **QR Code Provisioning:** Backend endpoint `https://api.cdccreditsmart.com/app/?code=provision` returns JSON with provisioning configuration including APK download URL and checksum. APK hosted at `https://api.cdccreditsmart.com/downloads/app-release.apk`. See `QR_CODE_BACKEND_INTEGRATION.md` for implementation details.
