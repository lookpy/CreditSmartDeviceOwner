...
## Overview
The Credit Smart Android App is a secure Device Owner application for mobile financial transactions, designed for Credit Smart clients. It provides advanced security, robust device pairing, real-time communication, and supports PIX and Boleto payments. The app aims to offer a secure and reliable financial management tool, facilitating financial inclusion through strong device control, including progressive blocking, anti-tampering, post-factory-reset enrollment, and offline functionality.
...
## System Architecture
The application uses Clean Architecture, MVVM, and Jetpack Compose for UI, structured into modules like `app`, `data`, `network`, `domain`, `device`, `payments`, and `biometry`.

**UI/UX Decisions:**
The UI utilizes Jetpack Compose and Material 3 with a CDC institutional dark theme. It features a streamlined navigation and a `ModernHomeScreen` displaying customer/device info, contract codes, and access to terms, focusing on installment history and PIX payment options.

**Technical Implementations:**
- **Device Management:** Device Owner provisioning, auto-configuration, policy enforcement, runtime permission requests, and a multi-layered Keep Alive System. Supports post-factory-reset enrollment via Samsung Knox Mobile Enrollment (KME) and Android Zero-Touch Enrollment.
- **Security & Persistence:** Time synchronization for tamper detection, Persistent State Manager for factory reset survival, `SelfDestructManager` for remote uninstall, timeout-based recovery, `WorkPolicyManager` for enterprise security policies, and `EncryptedSharedPreferences` for sensitive data. Uses JWT authentication. Anti-removal and lock-down mechanisms prevent uninstallation, force stops, and data clearing, including Full Device Lock (Kiosk mode) and blocking dangerous installations.
- **Offline Capabilities:** Local storage for overdue calculations, application blocking, and authentication persistence. Progressive blocking is managed via `DebtAgingCalculator` and `OfflineEnforcementWorker` with clock manipulation protection.
...
- **Financial Features:** Progressive blocking of non-essential applications based on payment status (supporting v2.5 backend with 6 limitation levels, `BlockAllFlags`, and semantic exceptions for banking/email apps) using `setApplicationHidden()`. Includes a universal dismissible overdue reminder overlay and an integrated PIX payment system.
- **Enhanced Protections:** Blocks parental control apps, popular apps, system updates, and restricts power-saving modes. The `SettingsGuard System` monitors and prevents access to dangerous Android Settings, forces GPS active, and applies `DISALLOW_CONFIG_LOCATION`.
...
- **App Blocking Logic:** Includes a `CRITICAL_NEVER_BLOCK_PACKAGES` list and `safeSetApplicationHidden()`. `SettingsGuardService` intercepts blocked apps using `UsageStats` and `BlockedAppExplanationActivity` with throttling. Detects and closes blocked apps in split screen mode. Guard initialization is provisioning-safe and only activates after Device Owner provisioning is complete.
- **Initialization Optimization:** Critical services are prioritized for initialization; permissions at Priority 0, essential services at Priority 1, background tasks at Priority 2.
- **Permission Handling:** PermissionGateScreen explicitly shows missing permissions (USAGE_STATS, OVERLAY, battery optimization) for manual granting.
