...
- **Progressive Blocking System**: Implemented based on days of delay (7, 15, 30, 45, 60 days). It blocks specific app categories (PHOTOS, GAMES, SOCIAL_MEDIA) and eventually most apps except essentials (WhatsApp, banking, calls, SMS, email). Uses `DevicePolicyManager.setApplicationHidden()`. Exceptions include essential apps. Includes secure persistence, detailed logging, batch sending of events to the backend, push notifications for blocking level changes, and unblocking mechanisms upon payment.

## External Dependencies
- **CDC Credit Smart Backend API**: For APK authentication (`POST /api/apk/auth`), device status (`GET /api/apk/device/status`), installments data (`/api/apk/device/installments`), payment processing, heartbeat (`POST /api/apk/device/heartbeat`), blocking rules (`/blocking-rules`), blocking events (`/blocking-events`), and unblock requests (`/request-unblock`).
