...
import com.cdccreditsmart.network.api.CdcDeviceStatusResponse
...
fun DeviceStatusResponse.toDomain(): DeviceStatus = try {
    DeviceStatus(
        ...
        blockingLevel = null, // Server doesn't provide blocking level in current API
        blockingReason = null, // Server doesn't provide blocking reason in current API
        allowedActions = emptyList(), // Server doesn't provide allowed actions in current API
        blockedPackages = emptyList(), // Server doesn't provide blocked packages in current API
...
fun InstallmentInfo.toDomain(): Installment = try {
    ...
    Installment(
        ...
        contractId = "",
        number = 1,
        dueDate = LocalDate.now(), // Use current date as fallback when error occurs
